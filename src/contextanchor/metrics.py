...
class MetricsCollector:
    ...
    def get_events(
        self, repository_id: Optional[str] = None, event_types: Optional[List[str]] = None
    ) -> List[Dict[str, Any]]:
    ...
    def export_metrics(self, format: str = "json") -> str:
        ...
        if format == "json":
            ...
            return json.dumps({"events": events, "time_to_productivity": ttp_summary}, indent=2)

        elif format == "csv":
            output = io.StringIO()
            writer = csv.DictWriter(
                output, fieldnames=["timestamp", "event_type", "repository_id", "branch", "payload"]
            )
            writer.writeheader()
            for event in events:
