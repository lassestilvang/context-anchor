...
def test_cli_isolation_between_repos(mock_repo_a, mock_repo_b):
    ...
    # Mock dependencies
    with (
        patch("src.contextanchor.cli._find_git_root") as mock_find_root,
        patch("src.contextanchor.config.load_config") as mock_load_config,
        patch("src.contextanchor.git_observer.GitObserver") as mock_git_obs_class,
        patch("src.contextanchor.api_client.APIClient") as mock_api_client_class,
        ...
        mock_api_client.list_contexts.assert_called_with("id-repo-b", None, 20)
