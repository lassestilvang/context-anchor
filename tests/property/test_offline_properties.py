...
class TestOfflineQueueProperties:
    ...
    @given(operation_count=st.integers(min_value=1, max_value=250), payload=operation_payloads())
    ...
    @given(retry_count=st.integers(min_value=0, max_value=15), payload=operation_payloads())
    @settings(max_examples=100, deadline=1000)  # Allow up to 1 second per example
    def test_exponential_backoff_and_expiration(self, retry_count, payload):
        ...
        **Validates: Requirements 8.7**
        ...
        # Queue an operation
        op_id = self.storage.queue_operation(
            operation_type="save_context", repository_id=repository_id, payload=payload
        )
