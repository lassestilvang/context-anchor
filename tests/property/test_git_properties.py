...
def test_property_1_complete_commit_signal_capture(commit_data):
    ...
    **Validates: Requirements 1.1, 1.5, 1.6**
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert (
            commit_signal.hash == commit_hash
        ...
        assert (
            commit_signal.message == message.strip()
        ...
        assert isinstance(
            commit_signal.timestamp, datetime
        ), "Commit timestamp must be a datetime object"

        # Verify changed files are present
        assert hasattr(commit_signal, "files_changed"), "Commit signal must contain files_changed"
        assert isinstance(commit_signal.files_changed, list), "files_changed must be a list"

        # Verify all committed files are in the signal
        for filename in files:
            assert (
                filename in commit_signal.files_changed
...
def test_property_2_complete_branch_switch_signal_capture(branch_name):
    ...
    **Validates: Requirements 1.2, 1.5, 1.6**
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert "from_branch" in branch_signal, "Branch switch signal must contain from_branch"
        assert (
            branch_signal["from_branch"] == from_branch
        ), f"Source branch must match: expected '{from_branch}', got '{branch_signal['from_branch']}'"

        # Verify target branch is present and matches
        assert "to_branch" in branch_signal, "Branch switch signal must contain to_branch"
        assert (
            branch_signal["to_branch"] == branch_name
        ), f"Target branch must match: expected '{branch_name}', got '{branch_signal['to_branch']}'"

        # Verify timestamp is present and valid
        assert "timestamp" in branch_signal, "Branch switch signal must contain timestamp"
        assert isinstance(
            branch_signal["timestamp"], datetime
        ), "Branch switch timestamp must be a datetime object"

        # Verify repository identifier is present
        assert "repository_id" in branch_signal, "Branch switch signal must contain repository_id"
        assert isinstance(
            branch_signal["repository_id"], str
        ), "Repository identifier must be a string"
        assert (
            len(branch_signal["repository_id"]) == 64
        ), "Repository identifier must be 64 characters (SHA-256 hash)"
...
def test_property_3_complete_diff_signal_capture(file_modifications):
    ...
    **Validates: Requirements 1.3, 1.5, 1.6**
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert "file_paths" in diff_signal, "Diff signal must contain file_paths"
        assert isinstance(diff_signal["file_paths"], list), "file_paths must be a list"

        # Verify all modified files are in the signal
        expected_files = [filename for filename, _ in file_modifications]
        for filename in expected_files:
            assert (
                filename in diff_signal["file_paths"]
            ), f"File '{filename}' must be in file_paths list"

        # Verify change summary is present
        assert "files_changed" in diff_signal, "Diff signal must contain files_changed count"
        assert isinstance(diff_signal["files_changed"], int), "files_changed must be an integer"
        assert diff_signal["files_changed"] == len(
            expected_files
        ), f"files_changed count must match: expected {len(expected_files)}, got {diff_signal['files_changed']}"

        assert "lines_added" in diff_signal, "Diff signal must contain lines_added"
        assert isinstance(diff_signal["lines_added"], int), "lines_added must be an integer"

        assert "lines_deleted" in diff_signal, "Diff signal must contain lines_deleted"
        assert isinstance(diff_signal["lines_deleted"], int), "lines_deleted must be an integer"

        # Verify repository identifier is present
        assert "repository_id" in diff_signal, "Diff signal must contain repository_id"
        assert isinstance(
            diff_signal["repository_id"], str
        ), "Repository identifier must be a string"
        assert (
            len(diff_signal["repository_id"]) == 64
        ), "Repository identifier must be 64 characters (SHA-256 hash)"

        # Verify capture source is present and matches
        assert "capture_source" in diff_signal, "Diff signal must contain capture_source"
        assert (
            diff_signal["capture_source"] == "test"
...
def test_property_4_mixed_reference_extraction(message_data):
    ...
    **Validates: Requirements 1.4, 10.2, 10.3, 10.6**
    ...
    assert (
        extracted_pr_set == expected_pr_set
    ...
    assert (
        extracted_issue_set == expected_issue_set
...
def test_property_35_github_remote_parsing(url_data):
    ...
    **Validates: Requirements 10.1**
    ...
    assert result is not None, f"GitHub remote URL must be parsed successfully. URL: '{url}'"

    # Verify owner is extracted correctly
    assert hasattr(result, "owner"), "Parsed result must contain 'owner' attribute"
    assert (
        result.owner == expected_owner
    ), f"Owner must match. Expected '{expected_owner}', got '{result.owner}'. URL: '{url}'"

    # Verify repo name is extracted correctly
    assert hasattr(result, "name"), "Parsed result must contain 'name' attribute"
    assert (
        result.name == expected_repo
    ), f"Repo name must match. Expected '{expected_repo}', got '{result.name}'. URL: '{url}'"

    # Verify remote_url is preserved
    assert hasattr(result, "remote_url"), "Parsed result must contain 'remote_url' attribute"
    assert (
...
def test_property_35_non_github_urls_return_none(url):
    ...
    **Validates: Requirements 10.1**
    ...
    assert result is None, f"Non-GitHub URL must return None. URL: '{url}', got: {result}"
...
def test_property_35_all_url_formats_equivalent(owner_repo):
    ...
    **Validates: Requirements 10.1**
    ...
    for url in urls:
        result = github_integration.parse_remote_url(url)
        assert result is not None, f"URL format must be parsed successfully: '{url}'"
